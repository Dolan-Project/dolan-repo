...
const fragment=`precision highp float;uniform float time,yaw;uniform vec2 res;uniform sampler2D earth;
...
export function GlobeCanvas({searching=false,stopped=false}:{searching?:boolean;stopped?:boolean}){
 ...
 useEffect(()=>{const canvas=canvasRef.current,gl=canvas?.getContext("webgl",{alpha:true,antialias:true});if(!canvas||!gl)return;
  ...
  gl.uniform1i(gl.getUniformLocation(program,"earth"),0);const uTime=gl.getUniformLocation(program,"time"),uYaw=gl.getUniformLocation(program,"yaw"),uRes=gl.getUniformLocation(program,"res");let frame=0,yaw=0,rate=1,last=0;
  ...
  const render=(stamp:number)=>{resize();const dt=Math.min(.05,last?(stamp-last)*.001:0);last=stamp;rate+=(spinTarget.current-rate)*(spinTarget.current===0?.2:.09);if(Math.abs(rate-spinTarget.current)<.02)rate=spinTarget.current;yaw+=dt*IDLE_SPIN*rate;gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);gl.uniform1f(uTime,stamp*.001);gl.uniform1f(uYaw,yaw);gl.uniform2f(uRes,canvas.width,canvas.height);gl.drawArrays(gl.TRIANGLE_STRIP,0,4);frame=requestAnimationFrame(render)};frame=requestAnimationFrame(render);
