...
export function ProfileView({ user, action }: ProfileViewProps) {
  ...
  return <main className="mx-auto max-w-[1180px] px-margin pb-10 pt-4 md:px-margin-desktop md:pb-14 md:pt-6">
    <section className="relative rounded-[2rem] border border-outline-variant/50 bg-white p-3 shadow-[0_16px_45px_rgba(22,48,80,.09)] sm:p-4">
      <div className="relative h-52 w-full overflow-hidden rounded-[1.75rem] bg-surface-container-highest sm:h-72 lg:h-80">
        ...
        <span className="absolute right-4 top-4 inline-flex items-center gap-1.5 rounded-full bg-black/45 px-3 py-1.5 type-micro font-semibold text-white backdrop-blur-md"><Icon name="photo_camera" className="text-[14px]" />Cerita perjalanan</span>
        {action === "edit" && <Link href={ROUTES.profilEdit} className="absolute bottom-4 right-4 inline-flex items-center gap-1.5 rounded-full border border-white/30 bg-black/55 px-3 py-2 type-micro font-semibold text-white backdrop-blur-md"><Icon name="photo_camera" className="text-[14px]" />Ganti Sampul</Link>}
      </div>
    ...
    <section className="mt-8 grid gap-5 lg:grid-cols-[.75fr_1.55fr]">
      <div className="rounded-3xl border border-outline-variant/50 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-2"><p className="type-micro font-extrabold uppercase tracking-widest text-on-surface-variant">DolanScore & Trust</p><span className="chip bg-emerald-100 text-emerald-800"><Icon name="verified" /> Rekan Terpercaya</span></div><div className="mt-4 rounded-2xl bg-surface-container-low p-5 text-center ring-1 ring-primary/10"><div className="flex items-baseline justify-center gap-1"><Icon name="star" className="text-3xl text-amber-500" /><span className="text-5xl font-black tracking-tight text-on-surface">{rating}</span><span className="text-lg font-bold text-on-surface-variant">/ 5.0</span></div><p className="mt-2 type-caption text-on-surface-variant">Berdasarkan {user.rating.reviewCount} ulasan sesama rekan satu perjalanan</p></div><div className="mt-4 space-y-2.5"><TrustLine text="Rating hanya berasal dari peserta satu trip yang telah selesai" /><TrustLine text="Riwayat publik membantu calon teman perjalanan mengenal traveler" /></div></div>
      <div className="rounded-3xl border border-outline-variant/50 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3 border-b border-outline-variant/40 pb-4"><div><h2 className="type-subtitle">Penilaian dari Rekan Seperjalanan</h2><p className="mt-1 type-caption text-on-surface-variant">Nilai komunikasi dan sikap setelah menyelesaikan trip bersama.</p></div><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-fixed text-primary"><Icon name="monitoring" /></span></div><div className="mt-5 space-y-6"><RatingBar icon="forum" label="Komunikasi & Koordinasi" value={user.rating.communication} /><RatingBar icon="sentiment_satisfied" label="Sikap Ramah & Kebersamaan" value={user.rating.attitude} /></div><p className="mt-6 rounded-2xl border border-dashed border-primary/25 bg-surface-container-low px-4 py-3 type-caption text-on-surface-variant"><Icon name="link" className="mr-1 text-primary" />Setiap ulasan ditautkan ke trip publik yang benar-benar diikuti bersama.</p></div>
    </section>

    <section className="mt-8"><div className="flex items-center justify-between border-b border-outline-variant/60 pb-3"><h2 className="type-subtitle"><Icon name="explore" className="mr-2 text-primary" />Riwayat Trip Publik</h2><span className="type-caption text-on-surface-variant">{tripTotal} perjalanan</span></div>{tripTotal > 0 ? <div className="mt-4 grid gap-4 md:grid-cols-2"><HistoryCard image={ASSETS.komodo} place="Pulau Padar & Komodo" role="Host perjalanan" /><HistoryCard image={ASSETS.tanahLot} place="Pesisir Bali" role="Peserta perjalanan" /></div> : <div className="mt-4 rounded-2xl border border-dashed border-outline-variant bg-surface-container-low px-6 py-10 text-center"><Icon name="luggage" className="text-4xl text-primary" /><p className="mt-2 type-label">Belum ada riwayat trip publik</p><p className="mt-1 type-caption text-on-surface-variant">Trip selesai yang diizinkan tampil akan muncul di sini.</p></div>}</section>
  </main>;
}
