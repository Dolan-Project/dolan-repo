...
export function itineraryPinSvg(indexZeroBased: number, sequence: number, selected = false, colorOverride?: string) {
  const color = colorOverride ?? itineraryStopColor(indexZeroBased);
  const stroke = "#ffffff";
