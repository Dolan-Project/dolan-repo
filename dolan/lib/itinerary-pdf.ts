function escapePdf(text: string) {
  return text
    .replace(/[^\x20-\x7E]/g, (char) => (char === "—" || char === "–" ? "-" : char === "×" ? "x" : " "))
    .replace(/\\/g, "\\\\")
    .replace(/\(/g, "\\(")
    .replace(/\)/g, "\\)");
}
...
export function renderItineraryPdfBytes(title: string, lines: string[]): Uint8Array {
  ...
  for (const page of pageEntries) {
    ...
    objects.push(
      `${page.contentNum} 0 obj << /Length ${byteLength(page.content)} >> stream\n${page.content}\nendstream endobj`,
    );
  }
