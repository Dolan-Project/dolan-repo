...
import { placeFromTemplateStop } from "@/lib/template-itinerary";
...
export async function GET(request: Request, { params }: { params: Promise<{ templateId: string }> }) {
  ...
  return jsonResult({
    ...
    data: {
      ...
      coverPlace: placeFromTemplateStop(province.template.stops[0]?.name ?? province.name, province.name),
      transportMode: province.template.transportMode,
      days: Array.from({ length: province.template.durationDays }, (_, i) => ({
        id: `${province.slug}-${i + 1}`,
        dayNumber: i + 1,
        title: `Hari ${i + 1}`,
        stops: province.template.stops.filter((stop) => stop.day === i + 1).map((stop) => ({
          sequence: stop.sequence,
          activityType: "VISIT",
          customTitle: stop.name,
          durationMinutes: stop.durationMinutes,
          notes: stop.notes,
          place: placeFromTemplateStop(stop.name, province.name),
        })),
      })),
    },
  }, 200);
