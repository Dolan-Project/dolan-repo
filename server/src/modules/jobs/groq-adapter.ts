...
import { clampDailyStopBounds } from "./itinerary-optimize.ts";
...
export class GroqAdapter implements GenerationModel {
  ...
  async generate(input: { tripId: string; preferences?: Record<string, unknown> }): Promise<unknown> {
    ...
    const { minStops, maxStops } = clampDailyStopBounds(
      Number(input.preferences?.minStopsPerDay ?? 4),
      Number(input.preferences?.maxStopsPerDay ?? 6),
    );
...
Aturan rute wajib: buat tepat satu day card per hari perjalanan (dayNumber berurutan). Setiap hari HARUS terisi dari pagi sampai malam (mulai sekitar 08:00, selesai sekitar 20:00). DILARANG 1 destinasi per hari. Tiap hari WAJIB ${minStops}-${maxStops} tempat populer yang berdekatan (jalan kaki/ojek, koridor yang sama). Durasi kunjungan harus realistis: makan/warung/kafe 45-75 menit (DILARANG 3-5 jam di warung), museum/candi/keraton 90-150 menit, pantai/taman 90-180 menit. Kalau jam masih longgar, TAMBAH destinasi wisata terdekat — jangan memanjangkan waktu makan. SATU RENCANA TRIP: setiap destinasi maksimal dikunjungi SATU KALI. Dilarang mengulang tempat yang sama di hari berbeda (contoh: Gedung Sate hanya di satu hari). Kalau landmark besar (Danau Toba, Bukit Lawang), tambahkan 2-3 tempat LAIN di kawasan itu (desa, museum, pasar, air terjun terdekat), jangan pindah ke kota jauh dan jangan mengulang landmark yang sama. Semua place WAJIB di/dekat destinasi trip di preferences.destinationCity. DILARANG pindah pulau: Bali tidak boleh Labuan Bajo/Padar/Pink Beach/Komodo/NTT; Cirebon tidak boleh kosong atau jadi Bandung. Untuk Cirebon pakai Keraton Kasepuhan, Goa Sunyaragi, Batik Trusmi. Urutkan nearest-neighbor agar polyline Google Maps tidak bolak-balik. JANGAN mengarang travelDurationMinutes — isi 0 untuk stop pertama tiap hari dan null untuk stop berikutnya. Waktu tempuh dihitung dari jarak antar koordinat, bukan dari angka buatan. Sertakan estimasi TRANSPORT_LOCAL / ACTIVITIES / FOOD per kunjungan yang masuk akal untuk backpacker (warung, ojek/angkot, tiket lokal). Tiket destinasi berbayar (Dufan/Dunia Fantasi, museum, candi) wajib ada di ACTIVITIES dengan nominal. Destinasi laut/pulau/pentas pakai kapal/perahu, bukan mobil. Jarak antar kota >80 km pakai bus/travel, bukan ojek dikali kilometer.`,
        },
