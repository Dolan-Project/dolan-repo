...
const bodySchema = z.object({ points: z.array(z.object({ lat: z.number().min(-90).max(90), lng: z.number().min(-180).max(180) })).min(2).max(10) });
...
export function createRouteRouter() {
  ...
  router.post("/routes/preview", requireCapability("read_public"), async (req, res, next) => {
    try {
      ...
      if (!env.googleMapsServerKey) return res.status(503).json({ success: false, error: { code: "PROVIDER_UNAVAILABLE", message: "Google Routes API belum dikonfigurasi" } });
      const client = new GoogleRoutesClient(env.googleMapsServerKey);
      const segments = [];
      for (let index = 1; index < parsed.data.points.length; index += 1) {
        const leg = await client.computeLeg({ latitude: parsed.data.points[index - 1]!.lat, longitude: parsed.data.points[index - 1]!.lng }, { latitude: parsed.data.points[index]!.lat, longitude: parsed.data.points[index]!.lng });
        segments.push(leg);
      }
